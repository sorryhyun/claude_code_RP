...
class CacheManager:
    ...
    async def get_or_set_async(self, key: str, factory: Callable[[], Any], ttl_seconds: float = 60) -> Any:
        ...
        Returns:
            Cached or computed value
        """
        # Try to get from cache first (quick check without async lock)
        cached_value = self.get(key)
        if cached_value is not None:
            return cached_value

        # Use async lock for the compute-and-store operation
        # This prevents multiple coroutines from computing the same value
