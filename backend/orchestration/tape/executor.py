    ...
    async def execute(
        ...
        while not tape.is_exhausted():
            ...
            if room and room.is_paused:
                logger.info(f"Tape paused | Room: {orch_context.room_id}")
            ...
            try:
                ...
                logger.info(f"Tape interrupted | Room: {orch_context.room_id}")
    ...
    async def _execute_sequential(
        ...
        for agent in agents:
            ...
            if cell.cell_type == CellType.INTERRUPT and cell.triggering_agent_id == agent.id:
                logger.debug(f"Skipping self-interrupt for {agent.name}")
                continue
            ...
            try:
                ...
                if responded:
                    ...
                    skips += 1
                    logger.debug(f"Agent {agent.name} skipped")
    ...
    async def _execute_concurrent(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                skips += 1
                logger.debug(f"Agent {agents[i].name} skipped")
