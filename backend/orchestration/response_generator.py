...
"""
        ...
        logger.info(f"📝 Critic report saved to {report_path}")

    except Exception as e:
        logger.error(f"❌ Failed to save critic report: {e}")
...
class ResponseGenerator:
    ...
    async def _inject_memories_if_enabled(
        ...
        logger.info(
            f"🧠 [BRAIN MODE] Memory-brain enabled for {agent.name} with policy: {agent_config.memory_brain_policy}"
        )
        ...
        if not is_critic:
            ...
            logger.debug("📡 Broadcasting typing indicator for memory brain analysis")
        ...
        try:
            ...
            if memory_result.should_inject_now and memory_result.activated_memories:
                logger.info(f"💡 Injecting {len(memory_result.activated_memories)} memories for {agent.name}")
            ...
            logger.error(f"❌ Memory-brain error for {agent.name}: {e}")
